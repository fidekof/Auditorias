...
let select_user = document.getElementById("JUsername");
let select_pass = document.getElementById("JUpassword");
//const btn_search_user = document.getElementById("but_search_user");
//btn_search_user.addEventListener("click", ()=>{searchUser('Y')});
...
const getDataUserFetch = async () => {
    const username = document.getElementById("JUsername");
    const password = document.getElementById("JUpassword");
    const userL = select_user.value;
    const passL = select_pass.value;
    if ((username.value === null || username.value.length < 1 || username.value.trim().length < 1) && username.parentNode.style.display !== 'none') {
        M.toast({html: 'Debe ingresar Usuario', classes: 'rounded red lighten-1'});
        return {};
    }

    if ((password.value === null || password.value.length < 1 || password.value.trim().length < 1) && password.parentNode.style.display !== 'none') {
        M.toast({html: 'Debe ingresar Password', classes: 'rounded red lighten-1'});
        return {};
    }
    const url = getParamName(`${ambienteruta()}Login/Inicio/${userL}/${passL}`).replace("/Inicio/aud", "");
    const myHeaders = new Headers();
    myHeaders.append('Accept', 'application/json');
    myHeaders.append('Content-Type', 'application/json');
...
const searchUser = async (typeAud) => {
    const User = await getDataUserFetch();
...
function abrir() {
    ...
    window.location.href = "/Login/Menu/aud";
}
